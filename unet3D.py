...
from tensorflow.keras.layers import Conv3D, BatchNormalization, ReLU, Conv3DTranspose
...
class DoubleConvs(tf.keras.layers.Layer):
    ...
    def __init__(self, channels_out, ks):
        ...
        self.conv1 = Conv3D(filters=channels_out, kernel_size=ks, padding='same', kernel_initializer='he_normal')
        self.norm1 = BatchNormalization()
        self.conv2 = Conv3D(filters=channels_out, kernel_size=ks, padding='same', kernel_initializer='he_normal')
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, channels_in, channels_out, ks):
        ...
        self.down = Conv3D(filters=channels_out, kernel_size=ks, strides=2, padding='same', kernel_initializer='he_normal')
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, channels_in, channels_out, ks):
        ...
        self.up = Conv3DTranspose(filters=channels_out, kernel_size=ks, strides=2, padding='same', kernel_initializer='he_normal')
        self.norm = BatchNormalization()
        self.actv = ReLU()
...
class UNet(tf.keras.Model):
    ...
    def __init__(self, n_classes, filters_start=64, ks=4, depth=3):
        ...
        self.classifier = Conv3D(filters=n_classes, kernel_size=ks, padding='same', kernel_initializer='he_normal', activation='softmax')
